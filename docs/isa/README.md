...
![ROM](https://img.shields.io/badge/ROM-512%20rows-2563EB) ![Burn](https://img.shields.io/badge/Burn-61%20opcodes-DC2626)
...
- **Not a ~20-opcode lean map.** v1 ships **61 burned opcodes** (instruction set proper; idle `NOP` at row 0 sits beside them in the ROM). Unused rows are `status=nop` (no growth phantoms). The Dual-LUT **configuration space** (`256 × 256 × 8` carry selects) is much larger; that is not “524,288 installed instructions.”
