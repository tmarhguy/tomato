#!/usr/bin/env python3
"""Parse alu_ks synth.log and write reports/alu_ks_metrics.txt."""
...
OUT = REPORTS / "alu_ks_metrics.txt"
LIB = "sky130_fd_sc_hd__tt_025C_1v80"
NETLIST = "rtl/alu-32b-koggestone.v (+ support)"
TOP = "alu_32b_kogge_stone"
DESIGN = "Tomato dual-LUT ALU datapath + Kogge-Stone 32b add (same principle, fastest carry)"


def parse_timing_est() -> dict[str, str]:
    est_path = REPORTS / "alu_ks_timing_est.txt"
...
def main() -> int:
    ...
    subprocess.run(
        [sys.executable, str(Path(__file__).resolve().parent / "estimate_timing_alu_ks.py")],
        check=False,
    )
    ...
    lines = [
        f"# Benchmark metrics — {date.today().isoformat()}",
        f"design: {DESIGN}",
        f"netlist: {NETLIST}",
        f"top: {TOP}",
    ...
    lines.extend(
        [
            ...
            "",
            "## Timing (LUT mux + KS prefix liberty estimate)",
